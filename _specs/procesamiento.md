...
Mask R-CNN tiene su propia loss interna multitarea que no se modifica:

```
Loss interna Mask R-CNN = L_cls + L_bbox + L_mask
```

| Componente | Función |
|---|---|
| L_cls | Clasificación de la instancia detectada |
| L_bbox | Regresión del bounding box |
| L_mask | Segmentación binaria por instancia (máscara) |

No se agrega loss externa. Modificar la loss interna de Mask R-CNN requeriría alterar Detectron2 y desestabilizaría el transfer learning. Se confía en la loss interna + estrategia de descongelamiento progresivo para guiar el aprendizaje.

**nnU-Net:** gestiona su propia loss internamente de forma autoconfigurada.
...
**Weight decay:** aplicado directamente en el optimizador. Penaliza pesos muy grandes durante el entrenamiento previniendo overfitting sin modificar la arquitectura.

```
weight_decay = 1e-4
```

**Dropout:** no se agrega dropout extra. Los backbones preentrenados (ResNet, EfficientNet) ya incorporan su propio esquema de regularización internamente. Forzar dropout adicional desestabilizaría el transfer learning.
...
| Loss | Interna multitarea | Interna multitarea | Autoconfigurada | Interna multitarea |
...
## Métricas de evaluación obligatorias

### Consideración transversal: clases ausentes

Cuando una radiografía es de columna parcial, las vértebras fuera del campo no están en el ground truth. El modelo que no las predice no está cometiendo un error. Las métricas deben calcularse **excluyendo las clases ausentes** en cada imagen, no penalizándolas.

---

### Dice Coefficient por clase + mean Dice

Mide el overlap entre la máscara predicha y el ground truth. Métrica principal del proyecto en segmentación médica.

```
Dice = 2 × |Predicción ∩ GT| / (|Predicción| + |GT|)
Rango: [0, 1]  →  1 es overlap perfecto
```

- Se reporta **por clase** (una vértebra a la vez) para detectar qué vértebras aprende bien el modelo y cuáles falla.
- Se reporta como **mean Dice global** como resumen comparativo entre modelos.
- El promedio global puede ocultar que el modelo aprendió bien las lumbares pero falla en las cervicales: el Dice por clase es indispensable.
...
Métrica complementaria al Dice. Más estricta porque penaliza más los falsos positivos (sobrepredicción de bordes).

```
IoU = |Predicción ∩ GT| / |Predicción ∪ GT|
Rango: [0, 1]
```

- Si el Dice es alto pero el IoU cae significativamente, el modelo está sobreprediciendo.
- Se reporta por clase y como promedio global (mIoU).
...
### AP@50 + mAP — Average Precision (solo modelos de instancia)

Evalúa si el modelo detectó correctamente cada vértebra como instancia individual. Estándar de evaluación para modelos de segmentación por instancia.

```
AP@50:  detección correcta si IoU entre predicción y GT ≥ 0.50
mAP:    promedio de AP sobre todas las clases de vértebras
```

- AP@50 es el umbral de referencia principal.
- mAP resume la calidad de detección global del modelo.
- **No aplica a nnU-Net** porque es segmentación semántica, no por instancia.
...
### Resumen de métricas obligatorias por modelo

| Métrica | Modelo 1 | Modelo 2 | Modelo 3 | Modelo 4 |
|---|---|---|---|---|
| Dice por clase | Sí | Sí | Sí | Sí |
| mean Dice | Sí | Sí | Sí | Sí |
| mIoU | Sí | Sí | Sí | Sí |
| AP@50 + mAP | Sí | Sí | No | Sí |

---

## Estrategia de split del dataset

### Distribución

| Conjunto | Porcentaje | Imágenes aprox. | Uso |
|---|---|---|---|
| Train | 70% | 175 | Aprendizaje de pesos |
| Validación | 15% | 37 | Early stopping, checkpointing, monitoreo |
| Test | 15% | 38 | Evaluación final — no se toca hasta el final |

### Reglas del split

- **Estratificado por Normal/Scoliosis:** mantener la proporción 71/179 en cada conjunto para no sesgar ningún subconjunto hacia un tipo de caso.
- **El test set se reserva completamente** hasta que todos los modelos estén entrenados y seleccionados. Usarlo antes contamina la evaluación final.
- **Augmentation solo en train.** Validación y test usan imágenes sin augmentation para medir rendimiento sobre datos reales.

### Qué automatiza el conjunto de validación

El validation set no ajusta hiperparámetros por sí solo. Provee la señal para que los mecanismos automáticos ya diseñados operen:

```
Early stopping          → para la fase si val loss no mejora
ReduceLROnPlateau       → reduce LR si val loss se estanca
Model checkpointing     → guarda el mejor modelo según val loss
Avance entre fases      → descongelamiento progresivo según plateau
```

Los hiperparámetros de diseño (LR inicial, α/β de la loss, patience, arquitectura) se fijan manualmente antes de entrenar y no cambian durante el entrenamiento.

---

## Estructura de implementación

Cada modelo se implementa en un **notebook Jupyter independiente y autocontenido**. Cada notebook incluye su propio preprocesamiento, entrenamiento y evaluación. La duplicación de código entre notebooks es intencional: garantiza que cada experimento sea completamente aislado y ejecutable de forma independiente en cualquier máquina o entorno (local, Colab, Kaggle) sin depender de los demás.

| Notebook | Modelo |
|---|---|
| `maskrcnn_resnet50_fpn.ipynb` | Mask R-CNN + ResNet50-FPN |
| `maskrcnn_resnet101_fpn.ipynb` | Mask R-CNN + ResNet101-FPN |
| `maskrcnn_efficientnet_fpn.ipynb` | Mask R-CNN + EfficientNet-FPN |
| `nnunet.ipynb` | nnU-Net |
