...
**Estado:** Borrador — normalización pendiente de confirmar según arquitectura del modelo
...
### Paso 1 — Estandarización de resolución

**Aplica a:** imagen radiográfica + todas las máscaras asociadas.
...
### Paso 3 — ROI crop (Region of Interest)

**Aplica a:** imagen radiográfica + todas las máscaras.

- Usar la **máscara binaria** (`LabelBinaryJPG/`) para calcular el bounding box de la columna vertebral.
...
### Paso 6 — Normalización / Estandarización

**Estado: pendiente — a confirmar según arquitectura del modelo.**

- Si se usa **modelo preentrenado** (ej. encoder ResNet/EfficientNet en U-Net): usar estandarización `z-score`. Evaluar si usar stats de ImageNet (`mean=[0.485, 0.456, 0.406]`, `std=[0.229, 0.224, 0.225]`) o calcular stats propias del dataset de radiografías.
- Si se usa **arquitectura desde cero**: calcular media y desviación estándar del propio dataset de entrenamiento y estandarizar con esos valores.
...
Imagen RAW + Máscaras
        |
[1] Resize estandarizado (bilinear para imagen, nearest neighbor para máscaras)
        |
[2] Conversión a escala de grises (conversión perceptual L = 0.299R + 0.587G + 0.114B)
        |
[3] ROI crop con máscara binaria (con márgenes generosos)
        |
[4] CLAHE (solo imagen)
        |
[5] Augmentation sincronizada imagen-máscara
        |
[6] Normalización / Estandarización  ← PENDIENTE según modelo
        |
